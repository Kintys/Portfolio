(() => {
    ...
    function rippleEffectBlack() {
        document.addEventListener("click", (function(e) {
            const targetItem = e.target;
            if (targetItem.closest("[data-ripple-two]")) {
                const button = targetItem.closest("[data-ripple-two]");
                const ripple = document.createElement("span");
                const diameter = Math.max(button.clientWidth, button.clientHeight);
                const radius = diameter / 2;
                ripple.style.width = ripple.style.height = `${diameter}px`;
                ...
                "once" === button.dataset.ripple && button.querySelector(".ripple-black") ? button.querySelector(".ripple-black").remove() : null;
                button.appendChild(ripple);
                const timeOut = getAnimationDuration(ripple);
                setTimeout((() => {
                    ripple ? ripple.remove() : null;
    ...
    function initSliders() {
        if (document.querySelector(".content-testimonials__slider")) new core(".content-testimonials__slider", {
            ...
            loop: true,
            navigation: {
                prevEl: ".content-testimonials__arrow-prev",
                nextEl: ".content-testimonials__arrow-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1
